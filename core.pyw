...
hours = [12, 1, 2, 3, 4, 5, 6] # Controls how many hours there are in a night
...
def wlan_events(): ### Multiplayer feature, ignore please ###
    ...
    currentPower = open(PATH + "/data/time").read()
...
print("Flag bitcode: " + str(flags)) # Print screen config options
...
""" MAIN LOOP """
def GAME_LOOP():
    global running, passedFrames, cameraID, camera_active, currentTime, currentPower, camera_ui, camera_entities, office_ui, office_ui, camera_panel_display_entity, camera_panel_mmap_entity, office_display_entity, freddy_display_entity, djmm_display_entity,  leftDoorEntity, rightDoorEntity
    while running:
        ...
        for event in pygame.event.get():
            if event.type == KEYDOWN: # Handle keyboard events
                ...
                elif event.key == K_SPACE and int(currentPower) > 0: # Space pressed?
                    if camera_active:
                        camera_active = False
                ...
                elif event.key == K_a and camera_active == True: # Switch to previous camera
                    if (cameraID != 1):
                        cameraID -= 1
                    else:
                        cameraID = 6
                    camera_panel.showStaticAnimation(screen, clock) # Show static for a few seconds when switching cameras


                elif event.key == K_d and camera_active == True: # Switch to next camera
                    if (cameraID != 6): 
                        cameraID += 1
                    else:
                        cameraID = 1
                ...
                elif event.key == K_a and camera_active == False: # Toggle left door
                    if (open(PATH + "/data/leftDoorStatus", "r").read() == "open"):
                ...
                elif event.key == K_d and camera_active == False: # Toggle right door
                    if (open(PATH + "/data/rightDoorStatus", "r").read() == "open"):
                ...
                if camera_active == True:
                    ...
                    camera_panel.animatronicMovementDetected(screen, clock, cameraID)
        ...
        fonts.update(pixel_font, pixel_font_condensed, fpsCounter_font, screen, clock, camera_active, cameraID, currentPower, passedFrames) # Update on-screen text elements. In ui.fonts because this is messy
